...
def plot_benchmark_results(csv_file):
    ...
    # Tu CSV tiene: Greedy_Makespan, Meta_Makespan, Greedy_Time, Meta_Time, Jobs
    ...
    rects1 = ax1.bar(indices - width/2, df_grouped['Greedy_Makespan'], width, label='Greedy (LPT/Res)', color='#3498db')
    rects2 = ax1.bar(indices + width/2, df_grouped['Meta_Makespan'], width, label='Algoritmo Genético', color='#e74c3c')
    ...
    for i in indices:
        g_val = df_grouped.loc[i, 'Greedy_Makespan']
        ga_val = df_grouped.loc[i, 'Meta_Makespan']
        if ga_val < g_val:
    ...
    ax2.plot(df_grouped['Jobs'], df_grouped['Greedy_Time'], marker='o', label='Greedy Time', color='#3498db', linewidth=2)
    ax2.plot(df_grouped['Jobs'], df_grouped['Meta_Time'], marker='s', label='GA Time', color='#e74c3c', linewidth=2)
